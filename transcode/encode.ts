...
export async function encodeInputs(
  ...
  if (useCairoABI) {
    const inputs = rawInputs
      ? `${rawInputs.join(" ").split(",").join(" ")}`
      : "";
    return [func, rawInputs || [], inputs];
  }
...
export function encodeComplex(
  ...
  if (type.baseType === "array") {
    ...
    return [
      ...length,
      ...value.flatMap((val) =>
        encode_(type.arrayChildren, makeIterator([val]))
      ),
    ];
