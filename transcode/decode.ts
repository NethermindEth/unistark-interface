...
export async function decodeOutputs(
  ...
  const outputNodes: ParamType[] =
    FunctionFragment.fromObject(funcSignature).outputs ?? [];
  const outputs: string[] = rawOutputs ?? [];

  return decode_(outputNodes, makeIterator(outputs));
}
...
export function makeIterator(value: any) {
...
function numberIfSafe(n: bigint, width: number): BigNumber | number {
